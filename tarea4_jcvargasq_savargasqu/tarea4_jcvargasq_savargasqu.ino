...
void dispatch() {
  ...
  case s2:
    if (menuSel == MENU1)
      menu_selection(MENU1, COLOR_GREEN, length_MENU1); // menu1Fcn();
    if (menuSel == MENU2)
      menu_selection(MENU2, COLOR_YELLOW, length_MENU2); // menu2Fcn();
    if (menuSel == MENU3)
      menu_selection(MENU3, COLOR_MAGENTA, length_MENU3); // menu3Fcn();
    if (menuSel == MENU4)
      menu_selection(MENU4, COLOR_BLUE, length_MENU4); // menu4Fcn();
    if (flagMenuFinish) {
    ...
    if ((millis() - blink_millis) >= duration) {
      ...
      *fun_ptr(10);
      draw_selection(selection,
                     COLOR_BLACK); // tft.fillCircle(80, 64, 20, COLOR_BLACK);
    }
  } else {
    if ((millis() - blink_millis) >= duration) {
      blink_millis = millis();
